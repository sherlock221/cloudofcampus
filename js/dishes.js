Zepto(function($) {
	// 获得url参数
	var paramaters = getRequest();

    // 餐馆id
	var supplierId =  paramaters['supplierId'];
	// 餐馆名字
	var supplierName = decodeURI(paramaters['supplierName']);
	// 起送价
	var deliveryLeastValue = paramaters['deliveryLeastValue'];
	//运费
	var deliveryCharge = paramaters['deliveryCharge'];
	window.isRest =  paramaters['isRest'];
	window.pguid = paramaters['pguid'];
	var deviceId = paramaters['deviceId'];
	var cookieStr = getCookie('orderMsgs');
	if(cookieStr.length < 32){
		cookieStr = createCookieValue();
	}
	setCookie("orderMsgs", cookieStr, 3);
	...
	$.ajax({
		type : 'GET',
		datatype : 'json',
		url : getRootPath()+"/android/supplier!loadCategory.action?supplierId="+supplierId,
...
var product = {
		load : function(categoryId,page,category){
			$.ajax({
				type : 'GET',
				datatype : 'json',
				url : getRootPath()+"/android/supplier!loadProductByCategory.action?pageSize=" + 1000 + "&pageNum=" + 0+"&categoryId="+categoryId,
				success : function(data) {

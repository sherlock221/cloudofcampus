Zepto(function($) {
	...
	if (flag == "orderInfo" && !window.ismyRequest) {
		$('#pageone').hide();
		$('#pagetwo').show();
		...
		window.deviceId = stringToJson(getCookie('orderMsgs')).deviceId;

		myOrder.load(deviceId);
		...
		if (latitude == undefined && longitude == undefined) {
			navigator.geolocation
					.getCurrentPosition(function(position) {
						...
						var url = getRootPath()
								+ "/android/cloudOfCampus!loadRestaurantByLatitudeAndLongitude.action?latitude="
								+ latitude + "&longitude=" + longitude;
						restaurant.load(window.deviceId, url);
					});
			// latitude=34.2523800;
			// longitude=108.9897520;
		} else {
			var url = getRootPath()
					+ "/android/cloudOfCampus!loadRestaurantByLatitudeAndLongitude.action?latitude="
					+ latitude + "&longitude=" + longitude;
			...
			.click(
					function() {
						...
						if (flag == "orderInfo" && !window.isRequest) {
							...
							var url = getRootPath()
									+ "/android/supplier!loadSupplierByHotArea.action?pguid="
									+ pguid;
	...
	load : function(deviceId) {
		...
		$
				.ajax({
					type : "get",
					url : getRootPath()
							+ "/android/order!getOrdersByDeviceId.action?deviceId="
							+ deviceId,
					datatype : 'json',
					success : function(data) {
						if ("success" == data.errorCode) {
							...
							for (var i = 0; i < 5; i++) {
								...
								var orderContent = '<li class="order_item">'
										+ '<div class="res_info">'
										+ '<p class="res_title">'
										+ ord.supplier.companyName
										+ '</p>'
										+ '<p>'
										+ '<span>餐厅电话：</span> <span>'
										+ phone
										...
										+ '</p>'
										+ '</div>'
										+ '<div style="overflow: hidden;" class="res_info">';
								for (var j = 0;; j++) {
								...
								orderContent += '<p>'
										+ '<span class="foodname">配送费</span> <span class="order_num"></span>'
										+ '<span class="price">￥'
					...
					success : function(data) {
						...
						if (data.errCode) {
							...
							url = getRootPath()
								...
								+ latitude + "&longitude=" + longitude;
							restaurant.load(deviceId,url);
//							var message = '该校区暂未开通订餐服务，我们正在努力覆盖中';
//							alert(message);
							return false;
						}
						...
						for (var i = 0; i < ortherSuppliers.length; i++) {
							...
							var bb = $(
									'<a/>',
									{
										class : 'resitem_a',
										href : getRootPath()
												+ "/cloudofcampus/dishes.html?supplierId="
												+ supplierId + "&supplierName="
												+ encodeURI(supplierName)
												+ "&deliveryLeastValue="
												+ deliveryLeastValue
												+ "&deviceId=" + deviceId
												+ "&deliveryCharge="
												+ deliveryCharge + "&pguid="
												+ pguid
									});
							var logo;
							if (supplier.logoImg) {
